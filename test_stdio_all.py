...
def test_all_functions():
    ...
    for tool_name, params, description in tests:
        ...
        if 'error' in response:
            ...
            success = 'error' not in response.get('toolcall_result', {})
            if success:
                print(f"✅ 成功")
                if 'result' in response.get('toolcall_result', {}):
                    result_data = response['toolcall_result']['result']
                    if isinstance(result_data, list):
                        print(f"  结果数量: {len(result_data)}")
                    elif isinstance(result_data, dict):
                        if 'valid' in result_data:
                            print(f"  凭证状态: {'有效' if result_data['valid'] else '无效'}")
                        elif 'doc_id' in result_data:
                            print(f"  笔记ID: {result_data['doc_id']}")
            else:
                error_msg = response['toolcall_result'].get('error', '未知错误')
